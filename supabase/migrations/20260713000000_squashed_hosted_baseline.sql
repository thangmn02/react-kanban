...
-- plpgsql_check recognizes this no-op by name and uses its argument as an
-- ephemeral table declaration during static analysis. At runtime it returns 1.
CREATE OR REPLACE FUNCTION "app_private"."plpgsql_check_pragma"(VARIADIC "args" text[])
RETURNS integer
LANGUAGE sql
IMMUTABLE
SET "search_path" TO ''
...
ALTER FUNCTION "app_private"."plpgsql_check_pragma"(VARIADIC text[]) OWNER TO "postgres";
REVOKE ALL ON FUNCTION "app_private"."plpgsql_check_pragma"(VARIADIC text[]) FROM PUBLIC, anon, authenticated;
...
begin
  -- plpgsql_check cannot infer a temporary table created at runtime. During
  -- lint, give it an ephemeral table shape through the private no-op pragma.
  perform app_private.plpgsql_check_pragma(
    'table: pg_temp.task_position_updates(id uuid, list_id uuid, position integer)'
  );
...
-- Objects outside public/app_private are added from separately verified hosted
-- dumps. The Auth trigger came from hosted-auth-schema.sql. The task-covers
-- bucket settings and storage policies came from hosted storage metadata/schema.
...
-- Reassert the hosted function ACLs after local platform defaults have been
-- applied. Without these explicit revokes, a clean reset grants extra execute
-- privileges that are absent from the authoritative hosted schema.
REVOKE ALL ON FUNCTION "public"."accept_workspace_invite"(text) FROM anon;
REVOKE ALL ON FUNCTION "public"."can_edit_workspace"(uuid) FROM anon, authenticated;
REVOKE ALL ON FUNCTION "public"."can_manage_workspace_members"(uuid) FROM anon, authenticated;
REVOKE ALL ON FUNCTION "public"."create_workspace_with_owner"(text) FROM anon;
REVOKE ALL ON FUNCTION "public"."handle_new_user"() FROM anon, authenticated;
REVOKE ALL ON FUNCTION "public"."handle_new_workspace_owner"() FROM anon, authenticated;
REVOKE ALL ON FUNCTION "public"."invite_workspace_member_by_email"(uuid, text, text) FROM anon;
REVOKE ALL ON FUNCTION "public"."is_workspace_member"(uuid) FROM anon, authenticated;
REVOKE ALL ON FUNCTION "public"."rls_auto_enable"() FROM anon, authenticated;
REVOKE ALL ON FUNCTION "public"."update_task_positions"(jsonb) FROM anon;
